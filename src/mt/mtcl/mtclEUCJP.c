...
// PROJ-1755
static mtlNCRet mtlEUCJPNextChar( acp_uint8_t ** aSource, acp_uint8_t * aFence );

static acp_sint32_t mtlEUCJPMaxPrecision( acp_sint32_t aLength );
...
mtlNCRet mtlEUCJPNextChar( acp_uint8_t ** aSource, acp_uint8_t * aFence )
{
/***********************************************************************
 *
 ...
 ***********************************************************************/
    ...
    else if( mtlCodeSet3_First_BYTE( *(*aSource) ) )
    {
        if( aFence - *aSource > 2 )
        {
            if( (mtlCodeSet3_Second_BYTE( *(*aSource+1) ) == ACP_FALSE) ||

                (mtlCodeSet3_Third_BYTE( *(*aSource+2) ) == ACP_FALSE) )
            {
                sRet = NC_MB_INVALID;
                (*aSource) += 3;
            }
