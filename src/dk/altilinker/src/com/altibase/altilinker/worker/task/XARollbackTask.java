...
import com.altibase.altilinker.adlp.op.XARollbackOp;
import com.altibase.altilinker.adlp.op.XARollbackResultOp;
...
public class XARollbackTask extends Task
{
    ...
    public XARollbackTask()
    {
        super( TaskType.XARollback );
    }
    ...
    public void setOperation(Operation aOperation)
    {
        ...
        XARollbackOp sXARollbackOp = (XARollbackOp)aOperation;
       
        mTargetAndXidMap = new LinkedHashMap( sXARollbackOp.mXidCount );

        for ( int i = 0 ; i < sXARollbackOp.mXidCount ; i++ )
        {
            mTargetAndXidMap.put( sXARollbackOp.mTargetName[i],
                                  sXARollbackOp.mXid[i].convertToXid() );
        }
    }
    ...
    public void run()
    {
        ...
        XARollbackResultOp sResultOperation = (XARollbackResultOp)newResultOperation();
        ...
        else
        {
            ...
            while ( sIterator.hasNext() == true )
            {
                ...
                if (sRemoteNodeSession != null)
                {
                    ...
                    sRemoteNodeSession.executing( RemoteNodeSession.TaskType.XARollback );
    ...
    private RemoteNodeSession newRemoteNodeSession( Xid                aXid,
                                                    ...
                                                    XARollbackResultOp aResultOperation, 
    ...
    private void checkAndRollback( XAResource         aXARes,
                                   Xid                aXid,
    ...
    private void rollback( XAResource         aXARes,
                           Xid                aXid,
                           RemoteNodeSession  aRemoteNodeSession,
                           XARollbackResultOp aResultOperation )
    {
        int sResultCode = ResultCode.Success;
                
        try
        {
            aXARes.rollback( aXid );
            aRemoteNodeSession.executed();
        }
        catch ( XAException e )
        {
            sResultCode = e.errorCode;

            if ( sResultCode == XAException.XA_HEURRB )
            {
                try
                {
                    aXARes.forget( aXid );
            ...
            else if ( ( sResultCode == XAException.XA_HEURHAZ ) ||
                      ( sResultCode == XAException.XA_HEURCOM ) ||
                      ( sResultCode == XAException.XA_HEURMIX ) )
            {
                try
                {
                    mTraceLogger.log( e );
