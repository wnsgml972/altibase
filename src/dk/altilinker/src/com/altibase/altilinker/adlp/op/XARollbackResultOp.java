...
public class XARollbackResultOp extends ResultOperation
{   
    public LinkedList   mFailXidList       = new LinkedList();
    public LinkedList   mHeuristicXidList = new LinkedList();
    
    // ... Count of Failed XIDs (4)    XID1 (152)  ...Err Code XID2 (152)  Err Code
    
    public XARollbackResultOp()
    {
        super(OpId.XARollbackResult, false);
    }
...
//      writeInt( aOpPayload, mFailXidList.size() );
        ...
        writeInt( aOpPayload, mHeuristicXidList.size() );

        if( mHeuristicXidList.size() > 0 )
        {   
            sIter = mHeuristicXidList.iterator();
            while( sIter.hasNext() )
            {
                AldpXid sXid = (AldpXid)sIter.next();
                writeBuffer( aOpPayload, sXid.getByteBuffer() );
    ...
    {
        mHeuristicXidList.add( new AldpXid( aXid ) );
    }
}
