...
#define QUERY_FOR_VIEW "CREATE VIEW V$DBLINK_REMOTE_STATEMENT_INFO"          \
    "( TRANSACTION_ID, REMOTE_TRANSACTION_ID, STATEMENT_ID, QUERY,"          \
    "GLOBAL_TRANSACTION_ID  )"                                               \
    "AS SELECT "                                                             \
    " TRANSACTION_ID, REMOTE_TRANSACTION_ID, STATEMENT_ID, QUERY,"           \
    " GLOBAL_TRANSACTION_ID "                                                \
    "FROM X$DBLINK_REMOTE_STATEMENT_INFO"

/*
 * X$DBLINK_REMOTE_STATEMENT_INFO
 *
 *  TRANSACTION_ID ( integer )
 *  REMOTE_TRANSACTION_ID ( integer )
 *  STATEMENT_ID ( integer )
...
{
    dkmRemoteStatementInfo * sInfo = NULL;
    UInt sInfoCount = 0;
    UInt i = 0;
    
    IDE_TEST( dkmGetRemoteStatementInfo( &sInfo,
                                         &sInfoCount )
              != IDE_SUCCESS );
...
{
    (SChar *)"X$DBLINK_REMOTE_STATEMENT_INFO",
    dkivBuildRecord,
    gFixedTableColDesc,
    IDU_STARTUP_META,
    0,
...
IDE_RC dkivRegisterDblinkRemoteStatementInfo( void )
{
