...
#define QUERY_FOR_VIEW "CREATE VIEW V$DBLINK_LINKER_SESSION_INFO "        \
    "( SESSION_ID, STATUS, SESSION_TYPE ) "                               \
    "AS SELECT "                                                          \
    " SESSION_ID, STATUS, "                                               \
    " DECODE( SESSION_TYPE, 0, 'CONTROL', 1, 'DATA' ) SESSION_TYPE "      \
    "FROM X$DBLINK_LINKER_SESSION_INFO"                                  

/*
 * X$DBLINK_LINKER_SESSION_INFO
 *
 *  SESSION_ID ( integer )
 *  STATUS ( integer )
...
{
    dkmLinkerSessionInfo * sInfo = NULL;
    UInt sInfoCount = 0;
    UInt i = 0;

    IDE_TEST( dkmGetLinkerSessionInfo( &sInfo, &sInfoCount ) != IDE_SUCCESS );
...
{
    (SChar *)"X$DBLINK_LINKER_SESSION_INFO",
    dkivBuildRecord,
    gFixedTableColDesc,
    IDU_STARTUP_META,
    0,
...
IDE_RC dkivRegisterDblinkLinkerSessionInfo( void )
{
