 ...
 */
void unittestDkTestCheck()
{
    SInt sRet;

    ACT_CHECK(unittestDkGetOne() == 1);

    sRet = unittestDkGetOne();
    ACT_CHECK_DESC(sRet == 1, ("unittestDkGetOne() should return 1 but %d", sRet));
}

/*
 * ACT_ERROR demonstration function
 */
void unittestDkTestError()
{
    if (unittestDkGetOne() == 1)
    {
        ACT_CHECK(unittestDkGetOne() == 1);
    }
    else
    {
        ACT_ERROR(("Error: unittestDkGetOne() should return 1 but %d", unittestDkGetOne()));
    }
}
 ...
 */
void unittestDkTestAssert()
{
    SInt sRet;

    ACT_ASSERT(unittestDkGetOne() == 1);

    sRet = unittestDkGetOne();
    ACT_ASSERT_DESC(sRet == 1, ("unittestDkGetOne() should return 1 but %d", sRet));
}

/*
 * ACT_DUMP and ACT_DUMP_DESC demonstration function
 */
void unittestDkTestDump()
{
