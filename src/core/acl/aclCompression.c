...
/*
 **  Primitive C Datatypes for compatibility 
*/
typedef acp_bool_t   idBool;
#define ID_TRUE  ACP_TRUE     
#define ID_FALSE ACP_FALSE     

typedef acp_uint8_t  UChar;
typedef acp_uint16_t UShort;
typedef acp_uint32_t UInt;
typedef acp_uint64_t ULong;

typedef acp_sint8_t  SChar;
typedef acp_sint16_t SShort;
typedef acp_sint32_t SInt;
typedef acp_sint64_t SLong;

typedef acp_ulong_t vULong;
typedef acp_slong_t vSLong;

typedef acp_float_t  SFloat;
typedef acp_double_t SDouble;

typedef acp_rc_t IDE_RC;

#define IDE_EXCEPTION_CONT(a) a:   
#define IDE_EXCEPTION_END     ACE_EXCEPTION_END_LABEL:;
#define IDE_EXCEPTION(a)      goto ACE_EXCEPTION_END_LABEL; a:
#define IDE_SUCCESS           ACP_RC_SUCCESS
#define IDE_FAILURE           ACP_RC_EINVAL
#define IDE_TEST(expr)        if (expr) goto ACE_EXCEPTION_END_LABEL;
#define IDE_TEST_RAISE(expr, lab)        if (expr) goto lab;
#define IDE_RAISE(lab)             goto lab;

/*
**  Altibase Macros
*/
#define IDE_DASSERT ACE_DASSERT
#define IDE_ASSERT  ACE_ASSERT
#define ID_SIZEOF   sizeof
#define ID_ULONG    ACP_UINT64_LITERAL
#define ID_SLONG    ACP_SINT64_LITERAL
 ...
 *       len-1한 값을 적는다. 왜냐하면, m2헤더의 경우에는 그 첫 byte가 항상 64이상
 *       이어야 하기 때문이다. match의 length는 무조건 3이상이기 때문에 m2헤더는
 *       항상 64이상이 된다.
 ...
 *
 *
 * m3 => offset이 M2_MAX_OFFSET이하이면서 length가 M2_MAX_LEN 이상일 경우, 또는,
...
/*
 * #define IDU_COMPRESSION_CHECK_OVERFLOW (1)
 */
...
# else
#    define CHK_OVERFLOW(curr,end)  (curr)=(curr); (end)=(end);
                               ...
                               void*  aWorkMem )
{
     ...
     * 이전 버전에서는 여기에서 추가로 5를 제거 하였는데, 필요없다고 판단되어 삭제하였다.
     * UChar  *sSrcShortEnd = aSrcBuf + aSrcLen - M2_MAX_LEN - 5; 
     **********************************************************************/
    UChar  *sSrcShortEnd = aSrcBuf + aSrcLen - M2_MAX_LEN;
	 ...
	 ***********************************************************************/
        ...
        {
            --sSrcPtr;
            m_size = sSrcPtr - sSrcCalcPtr;

            IDE_ASSERT(m_size >= 3);
            IDE_ASSERT(m_size <= M2_MAX_LEN);

            if (m_off <= M2_MAX_OFFSET)
            {
                m_off -= 1;
                CHK_OVERFLOW(sDestPtr, sDestRealEnd);
                *sDestPtr++ = COMPRESSION_BYTE(((m_size - 1) << 5) | ((m_off & 7) << 2));
                CHK_OVERFLOW(sDestPtr, sDestRealEnd);
            ...
            else if (m_off <= M3_MAX_OFFSET)
            {
                m_off -= 1;
                CHK_OVERFLOW(sDestPtr, sDestRealEnd);
                *sDestPtr++ = COMPRESSION_BYTE(M3_MARKER | (m_size - 2));
                goto m3_m4_offset;
            }
            else
            {
        ...
        else
	{
		...
		if (m_size <= M4_MAX_LEN)
		{
		    IDE_ASSERT(m_size == M4_MAX_LEN);
                    CHK_OVERFLOW(sDestPtr, sDestRealEnd);
                    *sDestPtr++ = COMPRESSION_BYTE(M4_MARKER |
                                                   ((m_off & 0x4000) >> 11) | (m_size - 2));
                }
		     ...
		     ***********************************************************************/
                  m3_m4_len:
                    ...
                    IDE_ASSERT(m_size > 0);
                    CHK_OVERFLOW(sDestPtr, sDestRealEnd);
                    *sDestPtr++ = COMPRESSION_BYTE(m_size);
                }
            }
 ...
 *
 **********************************************************************/
ACP_EXPORT IDE_RC aclCompress(UChar   *aSrcBuf ,
                              UInt     aSrcLen,
                              UChar   *aDestBuf,
                              UInt     aDestLen,
                              UInt    *aResultLen,
                              void    *aWorkMem )
{
    ...
    *aResultLen = sDestPtr - aDestBuf;
    ...
    /* removing Codesonar warning : Unused value of sDestRealEnd */
    ACP_UNUSED(sDestRealEnd);
 ...
 **********************************************************************/
ACP_EXPORT IDE_RC aclDecompress(UChar       *aSrcBuf,
                                UInt         aSrcLen,
                                UChar       *aDestBuf,
                                UInt         aDestLen,
                                UInt        *aResultLen )
{
    register UChar *sDestPtr = aDestBuf;
    register UChar *sSrcPtr  = aSrcBuf;
    register UInt   t;
    register UChar *m_pos = NULL;
    ...
    *aResultLen = 0;
    ...
    if (*sSrcPtr > 17)
    {
        ...
        t = *sSrcPtr++ - 17;
        /**********************************************************************
         * 아래 문장에서 구지 t가 4보다 작은지 여부를 따질 필요가 없다. 이부분은 어짜피 literal
 ...
 eof_found:
    ...
    *aResultLen = sDestPtr - aDestBuf;
    ...
    {
        if (sSrcPtr < sSrcRealEnd)
        {
            /*
             * COMPRESSION_E_INPUT_NOT_CONSUMED
             */
        }
        else
        {
            /*
             * COMPRESSION_E_INPUT_OVERRUN
             */
        }
    }
