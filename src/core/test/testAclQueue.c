...
static acp_sint32_t testProducer(void *aArgs)
{
    ...
    {
        sRC = aclQueueEnqueue(&gQueue, (void *)i);
        ACT_CHECK_DESC(ACP_RC_IS_SUCCESS(sRC),
                       ("aclQueueEnqueue returned error %d (%u)", sRC, sThrId));
    }
...
static acp_sint32_t testConsumer(void *aArgs)
{
    ...
    while (1)
    {
        sRC = aclQueueDequeue(&gQueue, (void **)&sValue);
        ...
        {
            ACT_ERROR(("aclQueueDequeue returned error %d (%u)", sRC, sThrId));
        }
    }
...
static void testCheckResult(const acp_char_t *aTestName)
{
    ...
    ACT_CHECK_DESC(aclQueueIsEmpty(&gQueue) == ACP_TRUE,
                   ("Queue is not empty in %s", aTestName));
...
static void testSpinLockQueue(void)
{
    acp_time_t sTime = 0;
    acp_rc_t   sRC;

    sRC = aclQueueCreate(&gQueue, 1);
    ACT_CHECK(ACP_RC_IS_SUCCESS(sRC));
    ...
    testCheckResult("SpinLock Queue Unidirectional Test");
    ...
    testCheckResult("SpinLock Queue Bidirectional Test");

    aclQueueDestroy(&gQueue);

    if (gTimeCheck == ACP_TRUE)
    {
        (void)acpPrintf("thread %d, loop %d of spinlock queue: %5lld msec\n",
                        THREAD_COUNT,
                        LOOP_COUNT,
                        acpTimeToMsec(sTime));
    }
...
static void testLockFreeQueue(void)
{
    acp_time_t   sTime = 0;
    ...
    sRC = aclQueueCreate(&gQueue, ((sCPUCount > 1) ? -1 : 2));
    ACT_CHECK(ACP_RC_IS_SUCCESS(sRC));
    ...
    testInit();
    sTime += testUnidirectional();
    testCheckResult("LockFree Queue Unidirectional Test");
    ...
    testInit();
    sTime += testBidirectional();
    testCheckResult("LockFree Queue Bidirectional Test");

    aclQueueDestroy(&gQueue);

    if (gTimeCheck == ACP_TRUE)
    {
        (void)acpPrintf("thread %d, loop %d of lockfree queue: %5lld msec\n",
                        THREAD_COUNT,
                        LOOP_COUNT,
                        acpTimeToMsec(sTime));
    }
