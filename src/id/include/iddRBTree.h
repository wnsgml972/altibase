 ...
 *   $Id: 
 *   NAME
 *     iddRBTree.h - Red Black Tree
 *
 *   DESCRIPTION
 *     Red-Black Tree
 *
 *   Classes
 *     iddRBTree : Key/Data pair를 저장할 수 있는 Red-Black 트리
 *     iddRBLatchTree : iddRBTree에 latch를 활용한 동시성 제어 추가
 *
 *   MODIFIED   (04/07/2017)
 ********************************************************************** */

#ifndef O_IDD_RBTREE_H
#define O_IDD_RBTREE_H   1
...
public:
    const node* findFirstNode(void) const;
    const node* findNextNode(const node*) const;
...
public:
    /**
     * iddRBTree를 순회할 수 있는 iterator 클래스
    ...
    {
    public:
        iterator(const iddRBTree* aTree = NULL, const node* aNode = NULL)
            : mTree(aTree), mNode(aNode) {}
        inline iterator& moveNext(void)
        {
            mNode = mTree->findNextNode(mNode);
            return *this;
        }
...
class iddRBLatchTree : public iddRBTree
{
public:
...
typedef iddRBTree::iterator iddRBTreeIterator;

#endif /* O_IDD_RBTREE_H */
