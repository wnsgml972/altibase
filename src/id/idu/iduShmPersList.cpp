...
void iduCheckPersConsistency( iduStShmMemList * aStShmMemList )
{
    return;
...
#define iduCheckPersConsistency( xx )
#endif

iduShmPersList::iduShmPersList()
{
}

iduShmPersList::~iduShmPersList(void)
{
}
 ...
 * ---------------------------------------------------------*/
/*-----------------------------------------------------------
 * Description: iduShmPersList를 초기화 한다.
 *
 * aIndex              - [IN] 이 함수를 호출한 모듈
 *                            (어디서 호출하는지 정보를 유지하기 위해 필요)
 * aSeqNumber          - [IN] iduShmPersList 식별 번호
 * aName               - [IN] iduShmPersList를 호출한 scuMemPool의 이름.
...
/* ---------------------------------------------------------------------------
 * BUG-19253
 * iduShmPersList는 mFreeChunk와 mFullChunk의 두개의 chunk list를 유지한다.
                                ...
                                idBool               aBCheck )
{
    ...
    for( i = 0; i < sPartialChunkCnt ; i++ )
    {
        ...
        IDE_TEST( iduShmPersMgr::free( aStatistics, aShmTxInfo, sCur )
                  != IDE_SUCCESS );
...
/*
 * iduStShmMemChunk를 하나 할당하여, 이것을 iduShmPersList->mFreeChunk에 연결한다.
 */
                             ...
                             iduStShmMemList  * aMemListInfo )
{
    ...
    sChunkAddr = iduShmPersMgr::getAddrOfPtr( sNewChunk );
 ...
 *  Allocation
 * ----------------------------------------------*/
IDE_RC iduShmPersList::alloc( idvSQL          * aStatistics,
                              iduShmTxInfo    * aShmTxInfo,
                              iduStShmMemList * aMemListInfo,
                              idShmAddr       * aAddr4Mem,
                              void           ** aMem )
 ...
 *  Free for one
 * ----------------------------------------------*/
IDE_RC iduShmPersList::memfree( idvSQL          * aStatistics,
                                iduShmTxInfo    * aShmTxInfo,
                                idrSVP          * aSavepoint,
                                iduStShmMemList * aMemListInfo,
                                idShmAddr         aAddr4Mem,
...
IDE_RC iduShmPersList::cralloc( idvSQL          * aStatistics,
                                iduShmTxInfo    * aShmTxInfo,
                                iduStShmMemList * aMemListInfo,
                                idShmAddr       * aAddr4Mem,
                                void           ** aMem )
...
IDE_RC iduShmPersList::shrink( idvSQL          * aStatistics,
                               iduShmTxInfo    * aShmTxInfo,
                               iduStShmMemList * aMemListInfo,
                               UInt            * aSize )
{
...
UInt iduShmPersList::getUsedMemory( iduStShmMemList * aMemListInfo )
{
    UInt sSize;
...
void iduShmPersList::status( iduStShmMemList * aMemListInfo )
{
...
void iduShmPersList::dumpPartialFreeChunk4TSM( iduStShmMemList * aMemListInfo )
{
    iduStShmMemChunk * sMyChunk = NULL;
    UInt               sID = 0;
