    ...
    for (i = 0; i < loop; i++)
    {
        if (i == 0)
        {
            ...
            fprintf(stderr, "GETTIME(%s) : %010"ID_UINT64_FMT" %09"ID_UINT64_FMT"\n",
                    aMsg, (ULong)tp[i].tv_sec, (ULong)tp[i].tv_nsec);
        }
        else
        {
            ULong sGap;

            sGap = (tp[i].tv_sec - tp[i - 1].tv_sec) * ID_ULONG(1000000000) +
                (tp[i].tv_nsec - tp[i - 1].tv_nsec);
            
            fprintf(stderr, "%010"ID_UINT64_FMT"\n", sGap);
        }
    }
}
