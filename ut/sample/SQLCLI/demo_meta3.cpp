...
SQLRETURN execute_primary_keys(SQLHDBC dbc);
...
SQLRETURN execute_primary_keys(SQLHDBC dbc)
{
    ...
    SQLCHAR     pk_name[MAX_OBJECT_NAME_LEN + 1];
    SQLSMALLINT key_seq;
    ...
    rc = SQLPrimaryKeys(stmt,
                        NULL, 0,
                        NULL, 0,
                        (SQLCHAR *)"DEMO_META3", SQL_NTS);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLPrimaryKeys");
        goto EXIT_STMT;
    }
    ...
    rc = SQLBindCol(stmt, 5, SQL_C_SSHORT, &key_seq, 0, &key_seq_ind);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
    }
    rc = SQLBindCol(stmt, 6, SQL_C_CHAR, pk_name, sizeof(pk_name), &pk_name_len);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
    ...
    printf("TABLE_NAME COL_NAME KEY_SEQ PK_NAME\n"
           "============================================\n");
