...
SQLRETURN execute_insert(SQLHDBC dbc)
{
    ...
    rc = SQLPrepare(stmt, (SQLCHAR *)"INSERT INTO DEMO_EX3 VALUES(?, ?, ?, ?, ?, ?)", SQL_NTS);
    if (!SQL_SUCCEEDED(rc))
    {
...
SQLRETURN execute_select(SQLHDBC dbc)
{
    ...
    SQLINTEGER           age[ARRAY_SIZE];
    SQL_TIMESTAMP_STRUCT birth[ARRAY_SIZE];
    SQLSMALLINT          sex[ARRAY_SIZE];
    double               etc[ARRAY_SIZE];

    SQLLEN               etc_ind[ARRAY_SIZE];
    ...
    rc = SQLPrepare(stmt, (SQLCHAR *)"SELECT * FROM DEMO_EX3", SQL_NTS);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLPrepare");
        goto EXIT_STMT;
    ...
    rc = SQLSetStmtAttr(stmt, SQL_ATTR_ROW_BIND_TYPE, SQL_BIND_BY_COLUMN, 0);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLSetStmtAttr");
        goto EXIT_STMT;
    }
    rc = SQLSetStmtAttr(stmt, SQL_ATTR_ROW_ARRAY_SIZE, (void*)ARRAY_SIZE, 0);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLSetStmtAttr");
        goto EXIT_STMT;
    }
    rc = SQLSetStmtAttr(stmt, SQL_ATTR_ROW_STATUS_PTR, (void*)status, 0);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLSetStmtAttr");
        goto EXIT_STMT;
    }
    rc = SQLSetStmtAttr(stmt, SQL_ATTR_ROWS_FETCHED_PTR, &fetched_ptr, 0);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLSetStmtAttr");
        goto EXIT_STMT;
    ...
    rc = SQLBindCol(stmt, 1, SQL_C_CHAR, id[0], sizeof(id[0]), NULL);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
    }
    rc = SQLBindCol(stmt, 2, SQL_C_CHAR, name[0], sizeof(name[0]), NULL);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
    }
    rc = SQLBindCol(stmt, 3, SQL_C_SLONG, &age[0], 0, NULL);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
    }
    rc = SQLBindCol(stmt, 4, SQL_C_TYPE_TIMESTAMP, &birth[0], 0, NULL);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
    }
    rc = SQLBindCol(stmt, 5, SQL_C_SSHORT, &sex[0], 0, NULL);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
    }
    rc = SQLBindCol(stmt, 6, SQL_C_DOUBLE, &etc[0], 0, etc_ind);
    if (!SQL_SUCCEEDED(rc))
    {
        PRINT_DIAGNOSTIC(SQL_HANDLE_STMT, stmt, "SQLBindCol");
        goto EXIT_STMT;
