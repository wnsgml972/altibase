...
void __fastcall TForm9::Button1Click(TObject *Sender)
{
	...
	if (ComboBox1->Items->Count == 0 ) {
		sprintf(query, "create tablespace %s datafile '%s' size %sM autoextend %s ",
		 ...
		 sprintf(query, "create tablespace %s datafile '%s' size %sM ",
						SPACENAME->Text.c_str(),
						ComboBox1->Items->Strings[0].c_str(),
						INITSIZE->Text.c_str() );
		 for (i = 1; i < ComboBox1->Items->Count; i++) {
			sprintf(temp, ", datafile '%s' size %sM ",
						ComboBox1->Items->Strings[0].c_str(),
						INITSIZE->Text.c_str() );
			strcat(query, temp);
		 }
		 sprintf(temp, "autoextend %s ", ONOFF);
		 strcat(query, temp);
...
{
    ComboBox1->Items->Add(DATAFILE->Text);	
}
//---------------------------------------------------------------------------
void __fastcall TForm9::ComboBox1DblClick(TObject *Sender)
{
