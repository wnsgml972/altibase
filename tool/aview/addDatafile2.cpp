...
SQLHENV env10= NULL;
SQLHDBC dbc10 = NULL;
SQLHSTMT stmt10 = NULL;

//---------------------------------------------------------------------------
void dbErrMsg10(SQLHENV _env, SQLHDBC _dbc, SQLHSTMT _stmt)
{
    SQLCHAR errMsg[4096];
	SQLSMALLINT msgLen;
	SQLINTEGER errNo;
...
//---------------------------------------------------------------------------
// DB Connect
int dbConnect10(AnsiString DSN)
{
	SQLCHAR USER[41];
    SQLCHAR PASSWD[41];
	...
	// 연결을 위한 메모리 할당.
	if (SQLAllocEnv(&env10) != SQL_SUCCESS)
	{
		ShowMessage("SQLAllocEnv Fail");
		return 0;
	}

	if (SQLAllocConnect(env10, &dbc10) != SQL_SUCCESS)
	{
	...
	// 진짜 연결해봅니다.
	if (SQLConnect(dbc10, DSN.c_str(), SQL_NTS, USER, SQL_NTS, PASSWD, SQL_NTS) != SQL_SUCCESS)
	{
		dbErrMsg10(env10, dbc10, SQL_NULL_HSTMT);
		freeDBHandle10();
		return 0;
	}
...
void __fastcall TForm10::Button1Click(TObject *Sender)
{
	...
	// connect db
	if (!dbConnect10(Form5->SERVERNAME->Caption)) {
        return;
	}

    // alloc statement
	if (SQLAllocStmt(dbc10, &stmt10) != SQL_SUCCESS)
	{
	...
	if (ListBox1->Items->Count == 0 ) {
		sprintf(query, "alter tablespace %s add datafile '%s' size %sM autoextend %s ",
		 ...
		 sprintf(query, "alter tablespace %s add datafile %s ",
						SPACENAME->Text.c_str(),
	...
	if (SQLExecDirect(stmt10, query, SQL_NTS) != SQL_SUCCESS)
	{
		dbErrMsg10(env10, dbc10, stmt10);
		SQLFreeStmt(stmt10, SQL_DROP);
		freeDBHandle10();
		return ;
	}
	...
	freeDBHandle10();
    //ShowMessage(query);
	ShowMessage("Add Datafile Success");	
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TForm10::ListBox1DblClick(TObject *Sender)
{
